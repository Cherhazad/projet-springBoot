...
public interface VilleRepository extends JpaRepository<VilleTP6, Integer> {
	...
	@Query("SELECT v FROM VilleTP6 v JOIN FETCH v.departement d WHERE v.nbHabitants > :min AND d.codeDept = :depCode")
	List<VilleTP6> findVillesDeptPopGreater(@Param("min") int min, @Param("depCode") String depCode);

	@Query("SELECT v FROM VilleTP6 v JOIN FETCH v.departement d WHERE v.nbHabitants BETWEEN :min AND :max AND d.codeDept = :depCode")
	List<VilleTP6> findVilleDeptPopBetween(@Param("min") int min, @Param("max") int max, @Param("depCode") String depCode);

	@Query("SELECT v FROM VilleTP6 v JOIN FETCH v.departement d WHERE d.codeDept = :depCode ORDER BY v.nbHabitants DESC")
